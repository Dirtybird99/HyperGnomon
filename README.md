...
- **Reorg truncate-replay (M2)**: detection is implemented (`SafeHeight` atomic, `CheckReorgAt` stub); automatic rewind is not. Measured reality (July 2026, `cmd/reorgwatch`, see [DOCS/BENCHMARKS.md](DOCS/BENCHMARKS.md)): reorgs are **shallow but routine** — depth ≤ 2 inside the `STABLE_LIMIT=8` bound, at ~1 per 100 blocks (≈ 1.8/hour), and invisible to the canonical DAG record (zero sideblocks across 23 days while live rewrites occur hourly). civilware/Gnomon has no reorg handling at all; operators recover via `pop <N>` CLI. Our `resync` subcommand serves the same operator path. Full truncate-replay is tracked for v1.x.
